...
/* helper function for threads for boundaryDetection*/
// hi is not inclusive
void detectionHelper(const size_t hi, const size_t lo, const std::vector<std::vector<double>>& xx, 
                     const std::vector<std::vector<double>>& yy, std::vector<std::pair<double, double>> &result, 
                     const size_t cols, const svm_model* model) {
    ...
    for (size_t row = lo; row < hi; ++row) {
        ...
        while (right - left > 1) {
            ...
            if (left_point != right_point) {
                local_points.emplace_back(xx[row][mid + 1], yy[row][mid + 1]);
                right = left;
            }
    ...
    result.insert(result.end(), local_points.begin(), local_points.end());
}
...
conesList boundaryDetection(const std::vector<std::vector<double>>& xx, const std::vector<std::vector<double>>& yy, 
    ...
    size_t chunk_length = rows / NUM_THREADS;

    for (size_t i = 0; i < NUM_THREADS; ++i) {
        size_t lo = chunk_length * i;
        size_t hi = lo + chunk_length;

        if (i == NUM_THREADS - 1) {
            hi = rows;
        } 

        // chat told me to do std::ref to prevent temp thread copies, confirm if this is right
        std::thread curr_thread (detectionHelper, hi, lo, xx, yy, std::ref(boundary_points), cols, model); 
        all_threads.push_back(curr_thread);
    }
