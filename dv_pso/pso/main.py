...
def main():
    ...
    if PLOT:
        ...
        plt.ion()
        for i in range(0, len(np.array(gs_history)), 1):
...
def sectors_to_racing_line(sectors:list, inside_points:list, outside_points:list):
    ...
    for i in range(len(sectors)):
        ...
        # Avoid division by zero if inside and outside points are the same
        if length == 0:
            xp, yp = x1, y1
        else:
            t = sectors[i] / length  # Normalized parameter between 0 and 1
            xp = x1 + t * dx
            yp = y1 + t * dy
